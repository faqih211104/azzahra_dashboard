<?php $this->load->view('Template/header'); ?>
        ...
        <div class="content-area">
            ...
            <div class="intro-y box p-5 mt-5">
                <div class="mb-4">
                    <h2 class="text-lg font-medium">Form Edit Mou</h2>
                    <p class="text-gray-600 text-sm">Harap isi dengan teliti dan berhati hati</p>
                </div>

                <form id="mouFormPage" method="post" action="<?= site_url('Mou/edit/' . $mou['mou_id']) ?>">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="file_name" class="input w-full border" placeholder="Contoh: Servis Laptop Asus 12-12-2025" value="<?= htmlspecialchars($mou['file_name']) ?>" required>
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">Tanggal *</label>
                            <input type="date" name="tanggal" id="tanggal" class="input w-full border" value="<?= $mou['tanggal'] ?>" required>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="customer" class="input w-full border" placeholder="Masukkan nama customer" value="<?= htmlspecialchars($mou['customer']) ?>" required>
                        </div>
                        <div class="col-span-2">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Pilih Pengantar Surat</label>
                            <div class="flex items-center mb-2">
                                <input type="radio" id="custom_intro" name="intro_type" value="custom" <?= (!isset($mou['intro_text']) || $mou['intro_text'] != "Dengan hormat,\nKami AZZAHRA COMPUTER & AUTHORIZED SERVICE CENTER, mengajukan penawaran harga sebagai berikut:") ? 'checked' : '' ?> class="mr-2">
                                <label for="custom_intro" class="text-sm">Custom</label>
                            </div>
                            <div class="flex items-center mb-2">
                                <input type="radio" id="template_intro" name="intro_type" value="template" <?= (isset($mou['intro_text']) && $mou['intro_text'] == "Dengan hormat,\nKami AZZAHRA COMPUTER & AUTHORIZED SERVICE CENTER, mengajukan penawaran harga sebagai berikut:") ? 'checked' : '' ?> class="mr-2">
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea name="intro_text" class="input w-full border" rows="4" placeholder="Masukkan pengantar surat"><?= isset($mou['intro_text']) ? htmlspecialchars($mou['intro_text']) : '' ?></textarea>
                        </div>
                        <div class="col-span-2">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Ketentuan</label>
                            <textarea name="terms" class="input w-full border" rows="6" placeholder="Masukkan ketentuan satu per baris"><?= isset($terms) ? htmlspecialchars($terms) : '' ?></textarea>
                    ...
                    <div class="mt-5">
                        ...
                        <div class="overflow-x-auto">
                            <table class="table" id="itemsTablePage">
                                ...
                                <tbody id="itemsTableBodyPage">
                                    <?php foreach ($items as $index => $item): ?>
                                    <tr id="itemRowPage<?= $index + 1 ?>">
                                        <td><?= $index + 1 ?></td>
                                        <td><input type="text" class="input w-full border" name="spesifikasi[]" value="<?= htmlspecialchars($item['spesifikasi']) ?>" required></td>
                                        <td><input type="number" class="input w-full border qty-input" step="0.01" min="0" name="qty[]" value="<?= rtrim(rtrim(number_format($item['qty'], 2, '.', ''), '0'), '.') ?>" required onchange="calculateTotalPage(<?= $index + 1 ?>)"></td>
                                        <td><input type="text" class="input w-full border harga-input" name="harga[]" value="<?= number_format($item['harga'], 0, ',', '.') ?>" required onchange="calculateTotalPage(<?= $index + 1 ?>)" onkeyup="formatCurrencyPage(this)"></td>
                                        <td class="total-cell" id="totalPage<?= $index + 1 ?>">Rp. <?= number_format($item['total'], 0, ',', '.') ?>,-</td>
                                        <td><button type="button" class="button border text-red-600" onclick="removeItemPage(<?= $index + 1 ?>)"><i data-feather="trash-2" class="w-4 h-4"></i></button></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                                <tfoot>
                                    <tr>
                                        ...
                                        <td class="font-bold" id="grandTotalPage">Rp. <?= number_format($mou['grand_total'], 0, ',', '.') ?>,-</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
